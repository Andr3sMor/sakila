...
prefix_input = 'fact_rental/'  # Ajusta según donde estén los datos de customer
bucket_output = 'cmjm_datalake'
prefix_output = 'dim_customer/'
...
def main():
    ...
    # --- EXTRAER Y TRANSFORMAR DATOS PARA Dim_Customer ---
    # Asegúrate de que las columnas existan en el DataFrame
    dim_customer_df = fact_rental_df[['customer_id', 'name']].drop_duplicates()

    # --- GUARDAR RESULTADO COMO PARQUET SNAPPY ---
    table = pa.Table.from_pandas(dim_customer_df, preserve_index=False)
    buffer_out = BytesIO()
    pq.write_table(table, buffer_out, compression='snappy')

    output_key = f"{prefix_output}dim_customer.snappy.parquet"
